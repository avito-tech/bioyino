...
use futures::future::{Future, IntoFuture, ok, join_all};
use futures::sync::mpsc::Sender;
use bincode::{self, Infinite};
use {Cache, CAN_LEADER, IS_LEADER, FORCE_LEADER};
use task::Task;
...
pub enum PeerCommand {
    #[structopt(name="leader_disable")]
    LeaderDisable,
    #[structopt(name="leader_enable")]
    LeaderEnable,
    #[structopt(name="force_leader")]
    ForceLeader,
...
T: AsyncRead + AsyncWrite,
{
    ...
    fn poll(&mut self) -> Poll<Option<Self::Item>, Self::Error> {
        let message = match try_ready!(self.inner.poll().map_err(|e| PeerError::Io(e))) {
            Some(buf) => Some(
                bincode::deserialize(&buf).map_err(|e| PeerError::Decode(e))?,
                ),
            None => None,
        };
...
T: AsyncRead + AsyncWrite,
{
    ...
    fn start_send(&mut self, item: Self::SinkItem) -> StartSend<Self::SinkItem, Self::SinkError> {
        ...
        match item {
            Some(item) => {
                let message = bincode::serialize(&item, Infinite).map_err(
                    |e| PeerError::Decode(e),
                    )?;
...
impl PeerServer {
    pub fn new(listen: SocketAddr, handle: &Handle, chans: &Vec<Sender<Task>>, nodes: &Vec<SocketAddr>) -> Self {
        Self {
            listen,
            nodes: nodes.clone(),
            handle: handle.clone(),
                ...
                reader
                    .map(move |m| match m {
                        PeerMessage::Snapshot(shot) => {
                            ...
                            handle.spawn(future.map_err(|e| {println!("error joining snapshot: {:?}", e);}));
                            None
                        }
                        ...
                        PeerMessage::Command(PeerCommand::ForceLeader) => {
                            ...
                            nodes.clone().into_iter().map(|node|{
                                let command = PeerCommandClient::new(node.clone(), &handle, PeerCommand::LeaderDisable)
                                    .into_future()
                                    .map_err(move |e|{println!("could not send command to {:?}: {:?}", node, e);})
                                    .then(|_|Ok(()));
...
impl PeerCommandClient {
    pub fn new(address: SocketAddr, handle: &Handle, command: PeerCommand) -> Self {
        Self {
            address,
            handle: handle.clone(),
            command,
...
impl IntoFuture for PeerCommandClient {
    ...
    fn into_future(self) -> Self::Future {
        ...
        .and_then(move |codec| if resp_required {
            let resp = codec
                .into_future()
                .and_then(move |(status, _)| {
                    if let Some(PeerMessage::Status(status)) = status {
                        ...
                        println!("Unknown response from server: {:?}", status);
                    }
                    Ok(())
                })
            .then(|_| Ok(()));
            //Box::new(resp) as Self::Future type Future = Box<Future<Item = Self::Item, Error = Self::Error>>;
